...
export function resolveBin() {
  ...
  if (isWin) add(process.env.APPDATA && join(process.env.APPDATA, 'npm'));
    ...
    add(homedir() && join(homedir(), '.local'));
  }
...
export async function ask(q) {
  ...
  if (!repoAvailable) {
    // canLookUp:false — there is genuinely no repo here, so the chat turn must NOT offer to read code.
    // Without this the model happily answered "I can look through your git history, recent commits, or
    // code modifications" with the backend absent: grounded:false was set, but the sentence a human
    // actually reads promised a capability that did not exist.
    /* KEEP THE ROUTED TOOL. Hard-coding tool:'chat' here threw away what the user actually asked: a
     * call-trace question with no repo came back labelled 'chat', as if they had made small talk, so
     * the UI could no longer say "I understood you wanted a trace — I just have no repo indexed".
     * `chat:true` is what tells the UI to render this as a plain reply; `tool` stays the understanding. */
    const spoken = await chat(q, [], { canLookUp: false }).catch(() => null);
    if (spoken) return { answer: spoken, grounded: false, chat: true, tool, voiced: true, lang };
    // chat() returned nothing — be honest about why
    if (hasVoice()) {
      return { answer: `hmm, mon modèle n'a rien renvoyé 😿\n(try: toshi model minimax/minimax-m3  ·  or check your provider key)`, grounded: false, tool, chat: true, voiced: false, lang };
    }
    return { answer: `je n'ai pas de repo à regarder pour l'instant 👀\nmais je peux discuter ! installe un modèle d'abord :\n  toshi model minimax/minimax-m3\n\n(ou pointe-moi vers un repo indexé avec \`toshi\` dans ce dossier)`, grounded: false, tool, chat: true, voiced: false, lang };
  }
