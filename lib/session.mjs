...
import { speak, chat } from './llm.mjs';
...
export async function ask(q) {
  ...
  if (tool === 'help') {
    ...
    if (spoken) return { answer: spoken, grounded: false, chat: true, tool, voiced: true };
    // no voice configured (or the model call failed) → the honest abilities greeting, so the token-cheap
    // demo path still works and the user learns what the grounded side can answer.
    return { answer: `watching ${base} 👀 — ask me:\n• what changed\n• where is X\n• who calls X\n• architecture\n(je parle français aussi — "quoi de neuf", "où est X", "qui appelle X")\n(pas une question sur le code ? on peut aussi juste discuter 😺)`, grounded: false, tool };
  }
  try {
    ...
    return { answer: spoken || facts, grounded: true, tool, voiced: !!spoken };
  } catch (e) {
    if (e && e.code === 'ENOENT')
