...
import { speak, chat, hasVoice } from './llm.mjs';
  ...
  if (tool === 'help') {
    ...
    if (hasVoice()) {
      ...
      return { answer: `hmm, mon modèle n'a rien renvoyé (${lastLang === 'fr' ? 'appel LLM échoué' : 'LLM call failed'}) 😿\n`
        ...
        + 'I can still answer about the code — ask "what changed" / "où est X".', grounded: false, tool, chat: true, voiced: false };
    }
    ...
    return { answer: `watching ${base} 👀 — ask me:\n• what changed\n• where is X\n• who calls X\n• architecture\n(je parle français aussi — "quoi de neuf", "où est X", "qui appelle X")\n(pas une question sur le code ? installe un modèle — \`toshi model <id>\` — pour discuter aussi 😺)`, grounded: false, tool };
  }
  try {
