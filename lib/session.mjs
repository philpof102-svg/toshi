...
// Windows gotcha: execFile can't run the npm `.cmd` shim (spawn EINVAL since Node 18) — resolve the real
// exe at npm's standard global location instead. POSIX resolves the plain name fine.
function defaultBin() {
...
const BIN = process.env.CODEBASE_MEMORY_BIN || defaultBin();
