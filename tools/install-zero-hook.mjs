...
const scopeDir = args.includes('--project') ? join(process.cwd(), '.zero') : join(homedir(), '.config', 'zero');
...
console.log(args.includes('--remove')
  ...
  : `🐈  Toshi will now float whenever zero starts (hook in ${file})\n    remove anytime: node tools/install-zero-hook.mjs --remove`);
